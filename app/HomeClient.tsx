...
export function HomeClient() {
  ...
  const copy = isEnglish
    ? {
        ...
        lead: "SelfReg AI gives adolescents a short, structured reflection cycle. A student can work independently or link their profile to a teacher; after that, the teacher can review all of that student's sessions in read-only mode.",
        ...
        teacherText: "A student enters a teacher code once to link their profile. The teacher can then use the read-only dashboard to notice patterns and discuss a next step.",
        ...
        teacherGuideSummary: "A student uses the teacher code to link their profile to one teacher.",
        teacherGuideDetail: "After the connection, the teacher sees all of the student's existing and future sessions and analytics in read-only mode. Removing a student only removes that connection; it does not delete the student's sessions.",
        ...
        guides: {
          student: {
            ...
            lead: "You can use the practice independently. If you later link your profile to a teacher, your answers stay in your own dashboard and become visible to that teacher in read-only mode.",
          ...
          teacher: {
            ...
            lead: "The connection begins with the student. Once the student's profile is linked, the dashboard is a read-only view of all of that student's sessions.",
            ...
            steps: [
              ...
              ["02", "Share the code with the student", "The student enters the code in their dashboard to link their profile to you. One student can be linked to one teacher at a time."],
              ["03", "Review, then discuss a next step", "Use the complete session overview as a conversation prompt. Removing a student only removes the link from your dashboard; it never deletes the student’s sessions."],
            ],
          },
          api: {
            tab: "GitHub API key",
    ...
    : {
        ...
        lead: "SelfReg AI проводит подростка через короткий цикл рефлексии. Ученик может работать самостоятельно или привязать свой профиль к педагогу; после этого педагог видит все сессии ученика в режиме чтения.",
        start: "Начать сессию",
        ...
        teacherText: "Ученик один раз вводит код педагога и привязывает свой профиль. После этого педагог использует кабинет только для чтения: чтобы заметить паттерны и обсудить следующий шаг.",
        ...
        teacherGuideSummary: "Ученик вводит код педагога и привязывает к нему свой профиль.",
        teacherGuideDetail: "После подключения педагог видит все уже созданные и будущие сессии ученика и аналитику — только для чтения. Удаление ученика из кабинета снимает связь, но не удаляет его сессии.",
        ...
        guides: {
          student: {
            ...
            lead: "Практикой можно пользоваться самостоятельно. Если затем привязать профиль к педагогу, ответы останутся в личном кабинете и станут видны этому педагогу в режиме чтения.",
          ...
          teacher: {
            ...
            lead: "Подключение начинает ученик. После привязки профиля кабинет педагога показывает все сессии этого ученика в режиме чтения.",
            ...
            steps: [
              ...
              ["02", "Передайте код ученику", "Ученик вводит код в своём кабинете, чтобы привязать профиль к вам. Одновременно профиль можно привязать только к одному педагогу."],
              ["03", "Посмотрите сессии и обсудите следующий шаг", "Используйте полный обзор сессий как повод для разговора. «Убрать из моего кабинета» снимает только связь у педагога и никогда не удаляет сессии ученика."],
            ],
          },
          api: {
            tab: "Ключ GitHub API",
